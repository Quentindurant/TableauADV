...
// plus la colonne NO (bon de commande partenaire) apparue dans le Zoho.
const COLUMNS: ColumnSeed[] = [
  { key: 'impe',                label: 'IMPE',                             type: 'DATE',     position: 0,  width: 110 },
  // NO : n° de BC HIGHCOM ou code de lot EVERLINK (L1A/L1B/POC) — texte libre.
  { key: 'no',                  label: 'NO',                               type: 'TEXT',     position: 1,  width: 90 },
  { key: 'client',              label: 'CLIENT',                           type: 'TEXT',     position: 2,  width: 220 },
  { key: 'dpt',                 label: 'DPT',                              type: 'TEXT',     position: 3,  width: 70 },
  { key: 'cp_client',           label: 'CP CLIENT',                        type: 'TEXT',     position: 4,  width: 100 },
  { key: 'partenaire',          label: 'PARTE',                            type: 'SELECT',   position: 5,  width: 160 },
  { key: 'date',                label: 'DATE',                             type: 'DATE',     position: 6,  width: 110 },
  { key: 'porta_commentaires',  label: 'PORTA ET COMMENTAIRES IMPORTANT',  type: 'LONGTEXT', position: 7,  width: 320 },
  // La spec garde l'heure en texte libre (« 14H », « 14h »...) : type TEXT, pas TIME.
  { key: 'heure',               label: 'HEURE',                            type: 'TEXT',     position: 8,  width: 90 },
  { key: 'tech',                label: 'TECH',                             type: 'SELECT',   position: 9,  width: 130 },
  { key: 'nom_tech',            label: 'NOM TECH',                         type: 'SELECT',   position: 10, width: 160 },
  { key: 'nom_cp',              label: 'NOM CP',                           type: 'SELECT',   position: 11, width: 130 },
  { key: 'statut',              label: 'INSTALLATION',                     type: 'SELECT',   position: 12, width: 150 },
  { key: 'commentaires_planif', label: 'COMMENTAIRES PLANIF',              type: 'LONGTEXT', position: 13, width: 320 },
  { key: 'materiel_recu',       label: 'MATERIEL RECU',                    type: 'SELECT',   position: 14, width: 140 },
  { key: 'num_chrono',          label: 'N° CHRONO',                        type: 'TEXT',     position: 15, width: 120 },
  { key: 'infos_facturation',   label: 'INFOS FACTURATION',                type: 'TEXT',     position: 16, width: 220 },
];

// Statuts : palette douce validée par le responsable projet (contraste >= 4,5:1).
const STATUTS: ChoiceSeed[] = [
  { label: 'NEW',         bgColor: '#F7DC6F', textColor: '#6B5504', bold: true },
  { label: 'STAGING',     bgColor: '#F8B5C8', textColor: '#943126', bold: true },
  { label: 'A SUIVRE',    bgColor: '#FAD7A0', textColor: '#874D0B', bold: true },
  { label: 'ATT TECH',    bgColor: '#F8B5C8', textColor: '#943126', bold: true },
  { label: 'ATT PARTE',   bgColor: '#F8B5C8', textColor: '#943126', bold: true },
  { label: 'ATT PV',      bgColor: '#744388', textColor: '#FFFFFF', bold: true },
  { label: 'ATT 5 COM',   bgColor: '#F8B5C8', textColor: '#943126', bold: true },
  { label: 'ATT CLIENT',  bgColor: '#F8B5C8', textColor: '#943126', bold: true },
  { label: 'EN COLLECTE', bgColor: '#F9E79F', textColor: '#786208', bold: false },
  { label: 'STAND BY',    bgColor: '#85C1E9', textColor: '#002060', bold: true },
  { label: 'A PLANIFIER', bgColor: '#A3E4D7', textColor: '#0E6251', bold: true },
  { label: 'INSTALLATION', bgColor: '#9BDEB4', textColor: '#176638', bold: true },
  { label: 'A DISTANCE',  bgColor: null,      textColor: null,      bold: false },
  { label: 'ANNULEE',     bgColor: '#E6B0AA', textColor: '#78281F', bold: true },
...
// 41 partenaires (ordre de la spec §2.2). 6 couleurs figées (palette douce,
// texte accordé au fond) ; les 35 autres passent par pastelFor.
...
export async function seed(prisma: PrismaClient): Promise<void> {
  ...
  await upsertChoices(
    ...
    TECHS.map((label) => {
      if (label === 'DIRECT') {
        return { label, bgColor: null, textColor: '#0072A8', bold: true };
      }
      if (TECHS_VERTS.has(label)) {
        return { label, bgColor: null, textColor: '#196F3D', bold: true };
      }
      return { label };
    }),
  );
...
async function main(): Promise<void> {
  ...
  try {
    await seed(prisma);
    console.log('Seed terminé : 17 colonnes, 96 choix, 1 utilisateur.');
  } finally {
